...
**Status: Proposed（段階BのMarket Unitsから実装中）**
...
2026-09-08追記: 段階Aの[API契約](../reference/api-v1.md)を作成し、段階BのMarket Units GET/PUT・認証・WebUI・共通保存経路を実装した。External Assets / Portfolio Basisのv1、汎用取込、private連携は後続段階。以下の「設計開始時の構造」は移行前の記録である。
