...
**Status: Proposed（段階Bの3画面API・WebUIを実装。段階Cの数量取込を追加。対象拡張・private連携は後続）**
...
2026-09-10追記: [汎用数量取込API](../reference/quantity-import-api.md)を追加した。登録済みMarket Units数量に限り、入力元の対象制限、preview/commit、履歴・再送、手入力休止を実装する。以下の広いchange-setや現金取込は後続設計である。
