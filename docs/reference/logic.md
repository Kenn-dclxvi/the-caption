...
`MonthlyCurator.generate_v4_chronicle()` は v4.3 の月次ロジックであり、`daily_metrics` が15件以上存在する月に使用する。対象月の `ShadowLedger` 履歴を日付順に読み、先頭日と末尾日の資産クラス別評価額を `source + asset_class` 単位で比較する。加えて、`daily_metrics_YYYYMMDD.json` を集約して、月間総資産推移、寄与上位、欠損日、転換点候補を月次AIへ渡す。
...
LLM には `PROMPT_CHRONICLE_SYSTEM_V4` と `OUTPUT_SCHEMA_CHRONICLE_V4` を渡し、戻り値は `{"chronicle": ..., "meta": ...}` の形で V4 月次テンプレートへ接続できる。
月次の User データも `<daily_metrics_summary>`, `<daily_insights>`, `<monthly_trend_data>`, `<output_schema>` に分離し、`ABSOLUTE_AMOUNT` の増減は家計・運用の構造変化として扱う。
