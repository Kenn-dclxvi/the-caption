...
現在の入力元から外れたIDを含む履歴は制限付きクライアントへ返さない。所有者は履歴を取得できる。この対象制限は保存済みreceiptの再送応答にも適用する。対象範囲が維持されていれば、previewの正当な同一キー再送には元の結果を返す。設定版変更後のcommitは412とし、新しいpreviewを必要とする。cancelは設定版の一致を要求せず、現在の認証・権限・対象範囲とdiff_hashを確認する。所有者は対象変更前の未反映バッチも取り消せるが、確定済みバッチは取り消せない。
...
[入力元管理UI](../how-to/quantity-import.md)で登録・設定更新・休止解除・履歴照合・commit/cancelを行える。汎用の行追加・削除change-set、現金・取得原価の取込、日次の月別入力manifest、private Monexクライアント、定期実行は後続。今回の検証は架空の入力元と一時ファイルだけを使用する。
