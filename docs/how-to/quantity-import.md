...
「最新状態を取得」は編集中フォームを保持する。元設定の版が変わっていたら、その古いフォームをそのまま保存せず、編集を控えて最新設定からやり直す。通信断では元の要求を保持し、同じ要求を再送して結果を確認する。結果の期限切れでは最新の入力元・履歴と照合し、明示確認後に未送信編集を破棄して再開する。自動的に新キーで保存し直すことはない。
