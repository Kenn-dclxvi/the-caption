...
def test_refresh_with_end_date_passes_end_to_yf_download(tmp_path):
    ...
    with patch("src.domain.collection_history_updater.yf.download", return_value=_dummy_df()) as mock_dl:
...
def test_refresh_uses_us_market_date_for_us_stock(tmp_path):
    ...
    with (
        patch("src.domain.collection_history_updater.os.getenv", return_value=None),
        patch("src.domain.collection_history_updater.yf.download", return_value=_dummy_df()) as mock_dl,
    ):
        updater.refresh(target_date="2026-05-22", us_market_date="2026-05-21")
    _, kwargs = mock_dl.call_args
...
def test_us_stock_missing_expected_close_uses_alpha_vantage_fallback(tmp_path):
    ...
    with (
        ...
        patch("src.domain.collection_history_updater.yf.download", return_value=_missing_expected_close_df()),
        patch(
            "src.domain.collection_history_updater.urllib.request.urlopen",
...
def test_us_stock_empty_yfinance_result_uses_alpha_vantage_fallback(tmp_path):
    ...
    with (
        patch.dict(os.environ, {"ALPHA_VANTAGE_API_KEY": "demo"}),
        patch("src.domain.collection_history_updater.yf.download", return_value=pd.DataFrame()),
        patch(
            "src.domain.collection_history_updater.urllib.request.urlopen",
            return_value=_mock_alpha_vantage_response("123.45"),
        ) as mock_urlopen,
...
def test_us_stock_missing_close_column_uses_alpha_vantage_fallback(tmp_path):
    ...
    with (
        patch.dict(os.environ, {"ALPHA_VANTAGE_API_KEY": "demo"}),
        patch("src.domain.collection_history_updater.yf.download", return_value=_missing_close_column_df()),
        patch(
            "src.domain.collection_history_updater.urllib.request.urlopen",
            return_value=_mock_alpha_vantage_response("123.45"),
        ) as mock_urlopen,
...
def test_us_stock_empty_close_dataframe_uses_alpha_vantage_fallback(tmp_path):
    ...
    with (
        patch.dict(os.environ, {"ALPHA_VANTAGE_API_KEY": "demo"}),
        patch("src.domain.collection_history_updater.yf.download", return_value=_empty_close_dataframe_df()),
        patch(
            "src.domain.collection_history_updater.urllib.request.urlopen",
            return_value=_mock_alpha_vantage_response("123.45"),
        ) as mock_urlopen,
...
def test_us_stock_missing_expected_close_without_alpha_key_does_not_fallback(tmp_path):
    ...
    with (
        ...
        patch("src.domain.collection_history_updater.yf.download", return_value=_missing_expected_close_df()),
        patch("src.domain.collection_history_updater.urllib.request.urlopen") as mock_urlopen,
    ):
        updater.refresh(end_date="2026-05-21")
